...
public class Elevator implements GameObject, Drawable {
	 ...
	 */
	protected void drawLineBody(Graphics g, Body body,
			net.phys2d.raw.shapes.Line line) {
		//
		...
		Vector2f[] verts = line.getVertices(body.getPosition(),
				body.getRotation());
		g.drawLine((int) verts[0].getX(), (int) verts[0].getY(),
	...
	public void drawJoint(Graphics g, Joint j) {
		...
		if (j instanceof SlideJoint) {
			...
			g.drawLine((int) (p1.x + im.x * joint.getMinDistance()),
					(int) (p1.y + im.y * joint.getMinDistance()),
		...
		if (j instanceof AngleJoint) {
			...
			Vector2f VA = new Vector2f((float) Math.cos(RA),
					(float) Math.sin(RA));
			Vector2f VB = new Vector2f((float) Math.cos(RB),
					(float) Math.sin(RB));
			...
			g.drawLine((int) p1.x, (int) p1.y, (int) (p1.x + VA.x * 20),
					(int) (p1.y + VA.y * 20));
			g.drawLine((int) p1.x, (int) p1.y, (int) (p1.x + VB.x * 20),
					(int) (p1.y + VB.y * 20));
		}
		...
		if (j instanceof SpringJoint) {
			...
			g.drawLine((int) p2.getX(), (int) p2.getY(), (int) x2.getX(),
					(int) x2.getY());
		}
	}
